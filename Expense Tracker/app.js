...
const formatMoney = (money) => {
  return `\P${Number(money).toFixed(2)}`;
};

const removeDollar = (money) => {
  return Number(money.replace("P", ""));
};
...
const removeRow = (event) => {
  ...
  const removeTheseItems = document.querySelectorAll(
    `[data-expense="item-${id}"]`
  );
...
const displayExpenseRow = (expense) => {
  ...
  <div class="expense-table-amount" data-expense="item-${count}">${formatMoney(
    expense.amount
  )}</div>
